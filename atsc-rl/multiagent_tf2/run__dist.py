...
class Agent:
    ...
    def __init__(self, env_name, agent_num, action_sizes, state_sizes, ppo_config, problem_var, target_sas, args):
        ...
        self.active_area = [i for i in range(agent_num)]


    def set_active_area(self, index):
        
        index = index if isinstance(index, (list, tuple)) else [index]
        self.active_area = index
...
#            agent.replayNew()
    ...
    def train(self):
        ...
        for i, agent in enumerate(self.ppo_agent):
            if i in self.active_area: agent.replayNew()
...
def copy_agent(source, target):
    ...
    index = source.active_area[0]
    
    source_actor = source.ppo_agent[index].actor.model
    target_actor = target.ppo_agent[index].actor.model
    
    target_actor.set_weights(source_actor.get_weights())
    
    soruce_critics = source.ppo_agent[index].critic.models
    target_critics = target.ppo_agent[index].critic.models
    
    for source, target  in zip(soruce_critics, target_critics):
...
# we assume that there are two agents and two areas. 
# The active areas are mutually exclusive between the agents, 
# and each agent is responsible for one active area.
        
def trainSappo(args):
    
    #--target-TL 'SA 101,SA 104'
    env_0 = IsolatedEnv(args)
    env_1 = IsolatedEnv(args)
    
    config_0 = env_0.get_agent_configuration()
    config_1 = env_1.get_agent_configuration()
    
    agent_0 = Agent(*config_0)
    agent_1 = Agent(*config_1)
    
    agent_0.set_active_area(0) 
    agent_1.set_active_area(1)

    copy_agent(agent_0, agent_1)
    copy_agent(agent_1, agent_0)

    
    for trial in range(args.epoch):
        
        run_train_episode(trial, env_0, agent_0)
        run_train_episode(trial, env_1, agent_1)
